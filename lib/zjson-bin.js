...
(function() {
  ...
  function factory(_, lz4init, logger) {
    "strict mode";

    let lz4;

    // Since zJsonBin in synchronous suppose that in browser 
    // lz4 provided as initialized module from global scope (window)
    if (typeof lz4init === 'function') {
      const lz4Module = lz4init(); // expect sync init
      lz4 = lz4Module.lz4js;
    } else {
      // lz4 already initialized
      lz4 = lz4init;
    }
    ...
    function serialize(obj) {
      ...
      if (!zjsonbin.compressionThreshold || objString.length >= zjsonbin.compressionThreshold) {
        if (zjsonbin.debug === 2) {
  ...
  function registerLib(global, factory) {
    ...
    if (typeof exports === "object" && typeof module !== "undefined") {    
      ...
      logger.setLevel("all");
      // const lz4 = require.resolve('lz4-asm');
      // const lz = lz4 + '../dist/lz4wasm.sync';
      // console.log(lz);
      //const l2 = require("lz4-asm/../dist/lz4wasm.sync");
      // /Users/etelmon/projects/zjsonbin/node_modules/lz4-asm/dist/lz4wasm.js
      const l = require('lz4-asm/dist/lz4wasm.sync');
      factory(require("lodash"), l, logger);
