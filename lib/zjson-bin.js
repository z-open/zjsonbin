...
(function() {
  ...
  function factory(_, lz4, logger) {
    ...
    function serialize(obj) {
      ...
      if (objString.length >= 100000) {
        ...
        if (isBrowser && utf8TextEncoder) {
          ...
          // Server
          compressed = lz4.compress(Buffer.from(objString, "utf8"), lz4Opts);
        }
    ...
    function deserialize(binaryString) {
      ...
      try {
        ...
        try {
          if (binaryString.length > 3000) {
            memory(usedBefore);
          }
          compactJson = lz4.decompress(compactJson);
        } catch (err) {
          throw err;
        }
     ...
     * @return {Object} obj
     */
    async function deserializeAsync(binaryString) {
      if (zjsonbin.disabled || _.isEmpty(binaryString)) {
        return null;
      }
      const usedBefore = process.memoryUsage();
      ...
      try {
        ...
        try {
          ...
          const dt = compactJson;
          compactJson = await new Promise((resolve, reject) => {
            const stream = require('stream');

            // ---- using a file  for testing ---
            let fs = require('fs');
            fs.writeFileSync('test.dt', dt);
            const input = fs.createReadStream('test.dt', {highWaterMark: 4 * 1024});
            // OR ----- using the data for testing, more realistic
            // const input = new stream.PassThrough();
            // input.end(dt, {end: true});
            //
    ...
    function memory(usedBefore) {
      const used = process.memoryUsage();
      for (let key in used) {
          const nw = Math.round(used[key] / 1024 / 1024 * 100) / 100;
          const bf = Math.round(usedBefore[key] / 1024 / 1024 * 100) / 100;
          logger.debug(`${key} ${bf} MB ->  ${nw} MB`);
      }
    }
  ...
  function registerLib(global, factory) {
    ...
    if (source === "Server") {
      const lz4Module = require("./lz4wasm.sync")();
      lz4js = lz4Module.lz4js;
    }

    if (typeof exports === "object" && typeof module !== "undefined") {
      const lz4CommonJsModule = source === "Server" ? lz4js : require("lz4");
      // register for node
      logger = require("zimit-zlog").getLogger("zjsonbin");
      logger.setLevel("all");
      factory(require("lodash"), lz4CommonJsModule, logger);
